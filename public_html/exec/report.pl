...
if(&valid_email($email) == 1){
	...
	if($name ne ''){
		...
		if($#dbcount > 0){
			...
			my $involvement = "are involved with";
			if($q->param('distributes') && $q->param('views')){
				$involvement = "both distribute and view";
			} elsif($q->param('distributes')){
				$involvement = "distribute";
			} elsif($q->param('views')){
				$involvement = "view";
			}
			...
			print MAIL "It has recently been brought to our attention that you are, or have been, in violation of the Net Authority Acceptable Internet Usage Guidelines. It has been reported that you $involvement offensive materials over the Internet.\n\n";
			...
			print "<P>You must select which sections of the <a href=\"guidelines.html\">Net Authority Acceptable Internet Usage Guidelines</a> the offender has violated. Please use the back button on your browser and fill in the appropriate information.</P>\n";
		}
	}
	else {
		print "<P>You must provide the offender's full name. Please use your browser's back button and fill in the appropriate information.</P>\n";
	}
} else {
	print "<P>The offender's email address you have entered is not a valid email address. Please use your browser's back button to go back and check over the information you have entered.</P>\n";
}
